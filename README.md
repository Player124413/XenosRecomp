...
Vulkan vertex locations come from the `USAGE_LOCATIONS` table in `shader_recompiler.cpp`, which has to agree with the table the runtime assigns its vertex input bindings from: a shader that declares `[[vk::location(N)]]` for an element is fed by the input the runtime binds at location N, and a mismatch binds the wrong data without failing anywhere. The numbering follows the order in which a declaration lists its elements (`POSITION0`, `NORMAL0`, `TANGENT0`, `BINORMAL0`, `TEXCOORD0..3`, `COLOR0`, `BLENDINDICES0`, `BLENDWEIGHT0`, `COLOR1` and `TEXCOORD4..23` at 12..31), and a declaration with a usage that is not in the table is refused instead of guessed, so the failure names the usage rather than producing a shader that reads the wrong vertex data. A generic solution would assign unique locations per vertex shader and dynamically create vertex declarations at runtime.
